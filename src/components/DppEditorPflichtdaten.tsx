...
interface DppEditorPflichtdatenProps {
  dpp: Dpp
  isNew?: boolean
...
export default function DppEditorPflichtdaten({ dpp: initialDpp, isNew = false, onUnsavedChangesChange, availableCategories: propCategories, onSave: externalOnSave, onPublish: externalOnPublish, onDppUpdate }: DppEditorPflichtdatenProps) {
  ...
  // Load supplier invites (block-based, wie Merge 8de05e6)
  useEffect(() => {
    if (!dpp.id || dpp.id === "new") {
      console.log("[DppEditorPflichtdaten] loadInvites skipped - dpp.id:", dpp.id)
      return
    }

    async function loadInvites() {
      try {
        const response = await fetch(`/api/app/dpp/${dpp.id}/supplier-invites`, {
          cache: "no-store",
        ...
        if (response.ok) {
          ...
          setDataRequests(invites)
        } else {
          console.error("[DppEditorPflichtdaten] Failed to load supplier invites:", response.status)
        }
      } catch (error) {
        console.error("[DppEditorPflichtdaten] Error loading supplier invites:", error)
      }
    }
    loadInvites()
  }, [dpp.id])
  ...
  return (
    ...
    {!externalOnSave && !externalOnPublish && !templateLoading && (template || !isNew) && (
    <StickySaveBar
      status={saveStatus}
      lastSaved={lastSaved}
      onSave={handleSave}
    ...
    {template && (
      <SupplierInviteModal
          ...
          try {
            const response = await fetch(`/api/app/dpp/${dpp.id}/supplier-invites`, {
              ...
              body: JSON.stringify({
                ...
                mode: invite.mode || "contribute",
                selectedBlocks: invite.selectedBlocks,
                selectedFieldInstances: invite.selectedFieldInstances,
                sendEmail: invite.sendEmail !== false
              })
            })
            ...
            if (data.invite) {
              const listRes = await fetch(`/api/app/dpp/${dpp.id}/supplier-invites`, { cache: "no-store" })
              if (listRes.ok) {
                const listData = await listRes.json()
                setDataRequests(listData.invites || [])
              }
            }
          } catch (error: any) {
        ...
        onInviteSuccess={async () => {
          if (dpp.id && dpp.id !== "new") {
            try {
              const res = await fetch(`/api/app/dpp/${dpp.id}/supplier-invites`, { cache: "no-store" })
              if (res.ok) {
                const data = await res.json()
                setDataRequests(data.invites || [])
              }
            } catch (err) {
              console.error("[DppEditorPflichtdaten] Error reloading invites:", err)
            }
          }
        }}
        onSendPendingInvites={async () => {
          ...
          setSupplierInviteLoading(true)
          try {
            const response = await fetch(`/api/app/dpp/${dpp.id}/supplier-invites/send-pending`, {
              method: "POST",
            })
            ...
            const sent = data.sent ?? data.sentCount ?? 0
            showNotification(
              data.message || `${sent} E-Mail${sent !== 1 ? "s" : ""} erfolgreich versendet`,
              "success"
            )
        ...
        onRemoveInvite={async (inviteId: string) => {
          ...
          setSupplierInviteLoading(true)
          try {
            const response = await fetch(`/api/app/dpp/${dpp.id}/supplier-invites/${inviteId}`, {
              method: "DELETE"
            })
